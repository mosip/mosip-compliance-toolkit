-- This table has all the details for a test run for a given collection in compliance toolkit project
...
CREATE TABLE toolkit.test_run(
	...
	CONSTRAINT test_runid_pk PRIMARY KEY (id) 
);

CREATE INDEX IF NOT EXISTS idx_test_run_id ON toolkit.test_run USING btree (id);
CREATE INDEX IF NOT EXISTS idx_test_run_collection_id ON toolkit.test_run USING btree (collection_id);
CREATE INDEX IF NOT EXISTS idx_test_run_run_dtimes ON toolkit.test_run USING btree (run_dtimes);
CREATE INDEX IF NOT EXISTS idx_test_run_execution_dtimes ON toolkit.test_run USING btree (execution_dtimes);
...
COMMENT ON COLUMN toolkit.test_run.run_dtimes IS 'Run Dt Time: Timestamp when run was created.';
...
COMMENT ON COLUMN toolkit.test_run.is_deleted IS 'is Deleted :flag to store soft delete status';
COMMENT ON COLUMN toolkit.test_run.del_dtimes IS 'Deleted DateTimestamp : Date and Timestamp when record is deleted.';
