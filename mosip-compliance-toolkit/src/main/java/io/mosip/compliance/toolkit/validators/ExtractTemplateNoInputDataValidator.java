...
public class ExtractTemplateNoInputDataValidator extends SDKValidator {
	...
	public ValidationResultDto validateResponse(ValidationInputDto inputDto) {
		...
		try {
			...
			if (statusCode == 404) {
				...
				validationResultDto.setDescriptionKey("EXTRACT_TEMPLATE_NO_INPUT_DATA_VALIDATOR_001" + AppConstants.ARGUMENTS_DELIMITER + statusCode);
			} else {
				validationResultDto.setStatus(AppConstants.FAILURE);
				validationResultDto.setDescription("For invalid data, unexpected status code received:" + statusCode);
				validationResultDto.setDescriptionKey("EXTRACT_TEMPLATE_NO_INPUT_DATA_VALIDATOR_002" + AppConstants.ARGUMENTS_DELIMITER + statusCode);
			}
		} catch (Exception e) {
			validationResultDto.setStatus(AppConstants.FAILURE);
