...
public class SbiProjectService {
	...
	public ResponseWrapper<SbiProjectDto> addSbiProject(SbiProjectDto sbiProjectDto) {
		...
		try
		{
			...
			serviceError.setMessage(ToolkitErrorCodes.SBI_PROJECT_UNABLE_TO_ADD.getErrorMessage()+ " " + ex.getMessage());
			serviceErrorsList.add(serviceError);
			responseWrapper.setErrors(serviceErrorsList);
		}
	...
	/**
	* Verifies all values of SbiProjectDto.
	* validates ProjectType, SpecVersion, Purpose, DeviceType, DeviceSubType
	*
	* @param SbiProjectDto
	* @return boolean
	*/
