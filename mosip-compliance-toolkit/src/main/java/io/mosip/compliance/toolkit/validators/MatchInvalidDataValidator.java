	...
	public ValidationResultDto validateResponse(ValidationInputDto inputDto) {
		...
		try {
			...
			if (statusCode == 401 || statusCode == 405) {
				...
				validationResultDto.setDescriptionKey("MATCH_INVALID_DATA_VALIDATOR_001" + AppConstants.NEW_DELIMITER + statusCode);
			} else {
				validationResultDto.setStatus(AppConstants.FAILURE);
				validationResultDto.setDescription("For invalid data, unexpected status code received:" + statusCode);
				validationResultDto.setDescriptionKey("MATCH_INVALID_DATA_VALIDATOR_002" + AppConstants.NEW_DELIMITER + statusCode);
			}
		} catch (Exception e) {
			validationResultDto.setStatus(AppConstants.FAILURE);
