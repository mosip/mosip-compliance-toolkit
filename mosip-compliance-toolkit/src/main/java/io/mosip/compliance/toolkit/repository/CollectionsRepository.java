...
public interface CollectionsRepository extends BaseRepository<CollectionEntity, String>{
	...
	@Query("SELECT e FROM CollectionEntity e  WHERE e.name= ?1 and e.sbiProjectId= ?2 and e.partnerId= ?3 and e.isDeleted<>'true' order by e.crDate desc")
	public List<CollectionEntity> getSbiCollectionByName(String name, String sbiProjectId, String partnerId);
	
	@Query("SELECT e FROM CollectionEntity e  WHERE e.name= ?1 and e.sdkProjectId= ?2 and e.partnerId= ?3 and e.isDeleted<>'true' order by e.crDate desc")
	public List<CollectionEntity> getSdkCollectionByName(String name, String sdkProjectId, String partnerId);

	@Query("SELECT e FROM CollectionEntity e  WHERE e.name= ?1 and e.abisProjectId= ?2 and e.partnerId= ?3 and e.isDeleted<>'true' order by e.crDate desc")
