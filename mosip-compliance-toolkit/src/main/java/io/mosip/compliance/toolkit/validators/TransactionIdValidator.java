...
public class TransactionIdValidator extends SBIValidator {
    ...
    public ValidationResultDto validateResponse(ValidationInputDto responseDto){
        ...
        try{
            ObjectNode methodRequest=(ObjectNode)objectMapperConfig.objectMapper()
                    .readValue(responseDto.getMethodRequest(),ObjectNode.class);
            ...
            ObjectNode captureInfoResponse = (ObjectNode) objectMapperConfig.objectMapper()
                    .readValue(methodResponseJson, ObjectNode.class);
            ...
            if (!arrBiometricNodes.isNull() && arrBiometricNodes.isArray()) {
                for (final JsonNode biometricNode : arrBiometricNodes) {
                    ...
                    if((requestTransactionId.asText()).equals(transactionId)){
                        ...
                        validationResultDto.setDescription("TransactionId Validation failed.");
                    }
                }
                if(result==true){
                    validationResultDto.setStatus(AppConstants.SUCCESS);
                    validationResultDto.setDescription("TransactionId validation is Successful");
                }
            }
